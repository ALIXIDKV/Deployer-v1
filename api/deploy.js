...
function parseFormData(req) {
  return new Promise((resolve, reject) => {
    ...
    busboy.on('file', (fieldname, file, filename, encoding, mimetype) => {
      ...
      file.on('end', () => files.push({ fieldname, filename, mimetype, buffer: Buffer.concat(chunks) }));
    });
...
async function cleanupProjectDir(dir) {
  try { await fs.rm(dir, { recursive: true, force: true }); } catch (e) { console.error('Gagal hapus temp:', e); }
}
...
async function getAllFiles(dir, basePath = dir) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      results = results.concat(await getAllFiles(fullPath, basePath));
    } else {
...
async function findDeployRoot(baseDir) {
  try { await fs.access(path.join(baseDir, 'index.html')); return baseDir; } catch {}
  const entries = await fs.readdir(baseDir, { withFileTypes: true });
  for (const entry of entries) {
    if (entry.isDirectory()) {
      const subPath = path.join(baseDir, entry.name);
      try { await fs.access(path.join(subPath, 'index.html')); return subPath; } catch {}
    }
  }
...
function validateZipEntries(zip) {
  for (const entry of zip.getEntries()) {
    if (entry.entryName.includes('..')) throw new Error('ZIP mengandung path traversal.');
  }
}
...
async function createGitHubRepo(owner, repoName, token) {
  const res = await fetch('https://api.github.com/user/repos', {
    method: 'POST',
    headers: { Authorization: `Bearer ${token}`, 'User-Agent': 'vanz-deployer', 'Content-Type': 'application/json' },
...
async function uploadFileToGitHub(owner, repo, filePath, contentBase64, token) {
  const res = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/${filePath}`, {
    method: 'PUT',
    headers: { Authorization: `Bearer ${token}`, 'User-Agent': 'vanz-deployer', 'Content-Type': 'application/json' },
    body: JSON.stringify({ message: `Add ${filePath}`, content: contentBase64, branch: 'main' }),
  });
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    throw new Error(`Gagal upload ${filePath}: ${err.message || res.statusText}`);
  }
}
...
async function deployToVercel(projectName, filesArray, token) {
  const res = await fetch('https://api.vercel.com/v13/deployments', {
    ...
    body: JSON.stringify({ name: projectName, files: filesArray, projectSettings: { framework: null }, target: 'production' }),
  });
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const repoName = rawName.trim()
      ...
      .replace(/^-|-$/g, '')
      .slice(0, 100);
    if (!repoName) return res.status(400).json({ success: false, message: 'Nama repo tidak valid.' });

    // Rate limit
    const limitResult = await checkDeployRateLimit(req, visitorId);
    if (limitResult && limitResult.blocked) {
      const remaining = limitResult.remainingSeconds;
    ...
    if (fileList.length > MAX_FILES_PER_DEPLOY) throw new Error(`Maks ${MAX_FILES_PER_DEPLOY} file per deploy.`);
    ...
    for (const file of fileList) {
      ...
      if (totalSize > MAX_TOTAL_EXTRACTED_SIZE) throw new Error('Total ukuran file melebihi 25 MB.');
      filesForUpload.push({ relativePath: file.relativePath, data: content.toString('base64') });
    }
    ...
    if (sessionId) markSessionFailed(sessionId);
