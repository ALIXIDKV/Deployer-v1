...
function memoryDel(key) { memoryStore.delete(key); }
function memoryExists(key) { return memoryGet(key) !== null; }
...
async function redisSet(key, value, ttlSeconds) {
  if (isRedisEnabled()) await redis.set(key, value, { ex: ttlSeconds });
  else memorySet(key, value, ttlSeconds);
}
...
export async function checkDeployRateLimit(req, visitorId) {
  ...
  for (const key of limitKeys) {
    if (key && (await redisExists(key))) {
      ...
      return { blocked: true, reason: 'cooldown', remainingSeconds: ttl > 0 ? ttl : DEPLOY_COOLDOWN_SECONDS };
    }
  }
  if (await redisExists(keys.activeDeploy)) {
    const ttl = await redisTTL(keys.activeDeploy);
    return { blocked: true, reason: 'active', remainingSeconds: ttl > 0 ? ttl : ACTIVE_DEPLOY_LOCK_SECONDS };
  }
  return null;
}
